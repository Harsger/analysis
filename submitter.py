...
def main(argv):
        ...
        filename = ''
        usage = 'submitter.py -c <\'command\'> -i <inputfile> -d <datapath> -t <treename> -s <specifier> -n <notuse> -e <eventsPerJob> -W -L -D'
        ...
        startPhrase += " --job-name="
        startPhrase += inputfile
        startPhrase += " --partition=ls-schaile "
        startPhrase += " --export=NONE "
        startPhrase += " --get-user-env "
        startPhrase += " --mem=3000mb "
        startPhrase += " --time=01:00:00 "
        startPhrase += " --output=/project/etpdaq4/CRF_data/nswQAQC/slurmLog/"
        startPhrase += inputfile
        startPhrase += "_log%j.txt "
        
        cwd = os.getcwd()
        startPhrase += " "
        startPhrase += cwd
        startPhrase += "/batcher.sh "
                ...
                if not wholeFile:
                    ...
                    for step in range(0,neededJobs):
                            ...
                            if jobstart > nevents:
                                    break
                            wasWaiting = False
                            while( not localAna and getJobCount(50,inputfile) ) :
                                sys.stderr.write('*')
                                time.sleep(1)
                                wasWaiting = True
                            if wasWaiting:
...
def getJobCount(limit,jobname):
    ...
    string = "squeue --user={user} --name={name} | wc -l".format(user=USER,name=jobname)
