...
void layerAlignment(
    ...
    for( auto d : moduleDesign ){
        ...
        fullCorrectionName = correctionFile;
        fullCorrectionName += "_";
        fullCorrectionName += d.first;
        
        vector< vector<string> > corrections = getInput( fullCorrectionName.Data() );
        ...
        for( auto c : corrections ){
            ...
            double number = atof( c.at(1).c_str() );
            stringstream value;
